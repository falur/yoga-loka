    ...
    public function handle(SendMessageCommand $command): void
    {
        ...
        $this->notificationSender->send(
            ...
            content: new NotificationContent(
                ...
                // снимок автора: id + имя + ссылка на аватар (клиент покажет аватар без запроса к профилю)
                actor: NotificationActor::of(userId: $senderId, name: $senderName, avatarUrl: $senderAvatarUrl),
...
- **`actor`** — от кого пришло уведомление: `NotificationActor::of(userId: ..., name: ..., avatarUrl: ...)`
  инициатора (например, того, кто подписался) либо `NotificationActor::none()` для системного
  уведомления. Ядро хранит **снимок** автора целиком — `userId`, имя и ссылку на аватар, — поэтому
  клиент показывает аватар автора **без отдельного запроса к профилю**. Снимок берётся на момент
  отправки: если автор позже сменит аватар, в старом уведомлении останется прежний.
...
```json
{
  ...
  "actor": {
    ...
    "name": "Иван Петров",
    "avatarUrl": "https://cdn.example/avatars/ivan.jpg"
  },
  "read": false,
...
пользователя, который подписался): `id` (его `userId`), `name` и `avatarUrl`. По этому снимку клиент
показывает аватар автора **без отдельного запроса к профилю**. Равен `null`, если у уведомления нет
...
- **Данные в push.** Через FCM `data` уезжает переход (`actionType` и `actionId`, если есть) и снимок
  автора (`actorId`, `actorName`, `actorAvatarUrl`, если автор есть) — FCM `data` плоская, поэтому
  снимок раскладывается по отдельным полям. Заголовок и тело идут в стандартный FCM `notification`.
...
- **Realtime-канал.** Публикация идёт в персональный канал `personal:#user_{userId}`. Payload —
  ...
  `{id, name, avatarUrl}` или `null`), `createdAt` (ISO-8601).
