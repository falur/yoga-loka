...
/**
 * Своя лента с cursor-пагинацией: владелец видит все свои записи, кроме заблокированных модерацией.
 ...
 */
final readonly class GetMyFeedHandler
{
    ...
    #[LogOperation]
    public function handle(GetMyFeedQuery $query): GetMyFeedResult
    {
        $viewer = UserId::fromString($query->authUserId);

        $page = $this->postReader->myFeed(
            ownerUserId: $query->authUserId,
            cursor: $query->cursor,
            limit: $query->limit,
        );

        return new GetMyFeedResult(
