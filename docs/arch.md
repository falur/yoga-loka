...
Command владеет границей транзакции и возвращает Result своего сценария. Query не меняет состояние и не открывает транзакцию записи: его handler читает свои данные через Reader, дочитывает чужие части через `Public` соседей и собирает Result.
...
Repository работает с агрегатом: читает и пишет таблицы своего агрегата, сколько бы их ни было, и возвращает Entity, коллекцию Entity, `null`, `bool` или `void`. Repository ничего не компонует и не имеет зависимостей, кроме собственной выборки, поэтому не может вызвать другой Repository, Reader, шину или соседний модуль.
...
Handler собирает ответ: Command handler изменяет состояние через Repository и Domain, Query handler получает Data от Reader и дочитывает недостающие части через `Public` соседей. Оба возвращают Result.
...
Промежуточной read-модели между Data и Result нет. Форма ответа сценария называется `{Action}Result` и лежит рядом со своим Command или Query, переиспользуемые части ответа — в `Application/Result`.
...
HTTP -> middleware -> Filter -> Controller -> QueryBus -> Query handler
     -> Reader и Public соседей -> Result
     -> Resource/Response -> JSON
