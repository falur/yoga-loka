...
- Маршрут события на Job-потребителя объявляет bootloader модуля-потребителя патчем секции конфигурации `outbox`: ключ — класс события, значение — непустой список маршрутов, каждый называет свой Job, очередь, список пауз повторов в секундах и таймаут доставки. Когда Job лежит в том же модуле, что и событие, объявление естественно оказывается в его bootloader; когда потребитель — другой модуль, объявление делает bootloader модуля-потребителя (он же импортирует свой Job), а не издателя: `Infrastructure/Spiral` одного модуля не видит `Infrastructure/Spiral` другого (deptrac), тогда как `Public/Event` соседа открыт всем — см. карточку [Job-потребитель](job-consumer.md).
