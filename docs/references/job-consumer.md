...
- Доставка повторяется, поэтому идемпотентность обязательна и реализуется в сценарии по идентификатору доставки, а не в Job.
- Потребитель не импортирует Domain, Application и Repository модуля-издателя: только его `Public/Event`.
- Пара «событие — Job» объявляется маршрутом outbox через `IntegrationEventRoutingContract`. Если событие и Job — разные модули, регистрацию делает bootloader модуля-потребителя (импортирует свой Job и чужой `Public/Event`), а не издателя: издатель не имеет доступа к `Infrastructure/Spiral` потребителя.
