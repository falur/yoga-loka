...
final class RequireNamedArgumentsRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if (
            $this->hasUnpackedArgument(arguments: $arguments)
            || $this->isVariadicCall(node: $node, scope: $scope)
            || $this->isPhpUnitAssertCall(node: $node, scope: $scope)
        ) {
            return [];
        }
    ...
    private function isPhpUnitAssertCall(Node $node, Scope $scope): bool
    {
        if (!$node instanceof StaticCall || !$node->class instanceof Name || !$node->name instanceof Identifier) {
            return false;
        }

        $className = $this->resolveStaticClassName(node: $node, scope: $scope);
        if ($className === null || !$this->reflectionProvider->hasClass(className: $className)) {
            return false;
        }

        $classReflection = $this->reflectionProvider->getClass(className: $className);
    ...
    private function resolveStaticClassName(StaticCall $node, Scope $scope): ?string
    {
        ...
        return match ($node->class->toString()) {
            'self', 'static' => $scope->getClassReflection()?->getName(),
            'parent' => $scope->getClassReflection()?->getParentClass()?->getName(),
            default => $scope->resolveName($node->class),
        };
    }
