...
namespace GianTiaga\SpiralApiErrors\Tests\Interceptor;
...
final class ApiExceptionInterceptorTest extends TestCase
{
    ...
    {
        $this->assertExceptionMapsToResponse(exception: new \DomainException(message: 'Некорректное значение', code: HttpStatus::UnprocessableEntity->value), expectedStatus: HttpStatus::UnprocessableEntity, expectedBody: '{"message":"Некорректное значение","code":422}', translator: self::englishTranslator());
    }
    public function testDomainExceptionWith401CodeMapsTo401(): void
    {
        $this->assertExceptionMapsToResponse(exception: new \DomainException(message: 'Нужна аутентификация', code: HttpStatus::Unauthorized->value), expectedStatus: HttpStatus::Unauthorized, expectedBody: '{"message":"Нужна аутентификация","code":401}', translator: self::englishTranslator());
    }
    public function testDomainExceptionWith403CodeMapsTo403(): void
    {
        $this->assertExceptionMapsToResponse(exception: new \DomainException(message: 'Доступ запрещён', code: HttpStatus::Forbidden->value), expectedStatus: HttpStatus::Forbidden, expectedBody: '{"message":"Доступ запрещён","code":403}', translator: self::englishTranslator());
    }
    public function testDomainExceptionWith404CodeMapsTo404(): void
    {
        $this->assertExceptionMapsToResponse(exception: new \DomainException(message: 'Не найдено', code: HttpStatus::NotFound->value), expectedStatus: HttpStatus::NotFound, expectedBody: '{"message":"Не найдено","code":404}', translator: self::englishTranslator());
    }
    public function testDomainExceptionWithOtherSupported4xxCodeMapsToSameCode(): void
    {
        $this->assertExceptionMapsToResponse(exception: new \DomainException(message: 'Конфликт', code: HttpStatus::Conflict->value), expectedStatus: HttpStatus::Conflict, expectedBody: '{"message":"Конфликт","code":409}', translator: self::englishTranslator());
    }
    public function testDomainExceptionWithClientCodeDoesNotLog(): void
    {
        $logger = new ApiExceptionInterceptorCountingLogger();
        $this->assertExceptionMapsToResponse(exception: new \DomainException(message: 'Некорректное значение', code: HttpStatus::UnprocessableEntity->value), expectedStatus: HttpStatus::UnprocessableEntity, expectedBody: '{"message":"Некорректное значение","code":422}', translator: self::englishTranslator(), logger: $logger);
        self::assertSame(0, $logger->recordsCount);
    }
    public function testDomainExceptionWithZeroCodeMapsTo500WithoutInternalMessage(): void
    {
        $this->assertExceptionMapsToResponse(exception: new \DomainException(message: 'Доменная ошибка'), expectedStatus: HttpStatus::InternalServerError, expectedBody: '{"message":"Internal server error","code":500}', translator: self::englishTranslator());
    }
    public function testDomainExceptionWithUnsupported4xxCodeMapsTo500WithoutInternalMessage(): void
    {
        $this->assertExceptionMapsToResponse(exception: new \DomainException(message: 'Неподдерживаемая ошибка', code: 499), expectedStatus: HttpStatus::InternalServerError, expectedBody: '{"message":"Internal server error","code":500}', translator: self::englishTranslator());
    }
    public function testUnexpectedExceptionMapsTo500WithEnglishMessageWithoutInternalMessage(): void
    {
        $this->assertExceptionMapsToResponse(exception: new \RuntimeException(message: 'SQL connection failed'), expectedStatus: HttpStatus::InternalServerError, expectedBody: '{"message":"Internal server error","code":500}', translator: self::englishTranslator());
    }
    ...
    {
        return new FakeTranslator(locale: 'en', messages: ['gian_tiaga.spiral_api_errors.internal_server_error' => 'Internal server error']);
    }
}
