...
namespace GianTiaga\SpiralApiErrors\Tests\Middleware;
...
final class RouteNotFoundMiddlewareTest extends TestCase
{
    ...
    {
        $this->assertRouteNotFoundResponse(translator: self::englishTranslator(), expectedBody: '{"message":"Route not found.","code":404}');
    }
    public function testRouteNotFoundExceptionReturnsRussianJson404(): void
    {
        $this->assertRouteNotFoundResponse(translator: new FakeTranslator(locale: 'ru', messages: ['gian_tiaga.spiral_api_errors.route_not_found' => 'Маршрут не найден.']), expectedBody: '{"message":"Маршрут не найден.","code":404}');
    }
    private function assertRouteNotFoundResponse(FakeTranslator $translator, string $expectedBody): void
    {
        ...
        $response = (new RouteNotFoundMiddleware(logger: new NullLogger(), translator: $translator))->process(request: $request, handler: new RouteNotFoundMiddlewareFixtureHandler(response: new Response(), exception: new RouteNotFoundException(uri: $request->getUri())));
    ...
    public function testSuccessfulResponsePassesThrough(): void
    {
        ...
        $response = (new RouteNotFoundMiddleware(logger: new NullLogger(), translator: self::englishTranslator()))->process(request: new ServerRequest(method: 'GET', uri: '/exists'), handler: new RouteNotFoundMiddlewareFixtureHandler(response: $expectedResponse));
        self::assertSame($expectedResponse, $response);
    }
    public function testOtherThrowableIsNotCaught(): void
    {
        $middleware = new RouteNotFoundMiddleware(logger: new NullLogger(), translator: self::englishTranslator());
        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage('Сервис недоступен');
        $middleware->process(request: new ServerRequest(method: 'GET', uri: '/broken'), handler: new RouteNotFoundMiddlewareFixtureHandler(response: new Response(), exception: new \RuntimeException(message: 'Сервис недоступен')));
    }
    public function testDebugLogDoesNotContainRequestValues(): void
    {
        $logger = new RouteNotFoundMiddlewareRecordingLogger();
        $request = new ServerRequest(method: 'POST', uri: '/hidden?token=query-secret', headers: ['Authorization' => 'Bearer header-secret', 'Cookie' => 'session=cookie-secret'], body: 'body-secret');
        (new RouteNotFoundMiddleware(logger: $logger, translator: self::englishTranslator()))->process(request: $request, handler: new RouteNotFoundMiddlewareFixtureHandler(response: new Response(), exception: new RouteNotFoundException(uri: $request->getUri())));
        self::assertCount(1, $logger->records());
        $record = $logger->records()[0];
        self::assertSame(LogLevel::DEBUG, $record->level);
        self::assertSame('HTTP-маршрут не найден.', $record->message);
        self::assertSame(['method' => 'POST', 'path' => '/hidden', 'status' => HttpStatus::NotFound->value, 'exceptionClass' => RouteNotFoundException::class], $record->context);
    ...
    {
        return new FakeTranslator(locale: 'en', messages: ['gian_tiaga.spiral_api_errors.route_not_found' => 'Route not found.']);
    }
}
...
final class RouteNotFoundMiddlewareRecordingLogger extends AbstractLogger
{
    ...
    public function log($level, string|\Stringable $message, array $context = []): void
    {
        ...
        $this->records[] = new RouteNotFoundMiddlewareLogRecord(level: (string) $level, message: (string) $message, context: $this->normalizeContext(context: $context));
    }
    /**
