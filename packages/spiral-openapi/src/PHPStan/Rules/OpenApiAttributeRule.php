...
final class OpenApiAttributeRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        foreach ($this->attributes($node) as $attribute) {
            ...
            if ($id === '') {
                $errors[] = RuleErrorBuilder::message('OpenApi id не должен быть пустым.')->identifier('gianTiaga.spiralOpenApi.emptyId')->build();
                continue;
            }
            if (!\preg_match(pattern: '/^[a-zA-Z0-9_.:-]+$/', subject: $id)) {
                $errors[] = RuleErrorBuilder::message(\sprintf('OpenApi id имеет недопустимый формат: %s.', $id))->identifier('gianTiaga.spiralOpenApi.invalidId')->build();
            }
            if (isset($this->ids[$id])) {
                $errors[] = RuleErrorBuilder::message(\sprintf('OpenApi id уже используется: %s.', $id))->identifier('gianTiaga.spiralOpenApi.duplicateId')->build();
            }
    ...
    private function attributeIsOpenApi(Attribute $attribute): bool
    {
        ...
        return $attributeName === 'OpenApi' || \str_ends_with(haystack: $attributeName, needle: '\OpenApi');
    }
