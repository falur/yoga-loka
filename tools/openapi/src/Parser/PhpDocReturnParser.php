...
use Tools\OpenApi\Model\GenericReturnType;
...
final readonly class PhpDocReturnParser
{
    ...
    public function parse(string|null $docComment, array $aliases, string $namespace): GenericReturnType|null
    {
        ...
        foreach ($phpDocNode->getReturnTagValues() as $returnTagValue) {
            ...
            return new GenericReturnType(
                wrapperClass: $this->resolveTypeName((string) $genericType->type, $aliases, $namespace),
                resourceClass: $this->resolveTypeName($this->lastIdentifier($resourceType), $aliases, $namespace),
            );
        }
    ...
    {
        $trimmedTypeName = \ltrim($typeName, '\\');

        if (isset($aliases[$trimmedTypeName])) {
            return $aliases[$trimmedTypeName];
        }

        if (\str_contains($trimmedTypeName, '\\')) {
            return $trimmedTypeName;
        }

        return \sprintf('%s\\%s', \trim($namespace, '\\'), $trimmedTypeName);
    }
