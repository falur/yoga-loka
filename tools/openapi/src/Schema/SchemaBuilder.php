...
namespace Tools\OpenApi\Schema;
...
final readonly class SchemaBuilder
{
    ...
    public function __construct(
        ...
        private array $classesByShortName,
    ...
    public function ensureClassSchema(ClassMetadata $classMetadata): void
    {
        ...
        if ($classMetadata->enum) {
            $this->schemaRegistry->add($classMetadata->shortName, [
                'type' => 'string',
                'enum' => $classMetadata->enumCases,
            ]);
        ...
        $this->schemaRegistry->add($classMetadata->shortName, $schema);
    }
    ...
    public function schemaForProperty(PropertyMetadata $propertyMetadata): array
    {
        if ($propertyMetadata->listItemType !== null) {
            return [
                'type' => 'array',
                'items' => $this->schemaForType($propertyMetadata->listItemType),
            ];
        }
