...
use Tools\OpenApi\Response\PaginationMetaResponse;
...
final class JsonResponseTest extends TestCase
{
    public function testPaginationResponseBuildsHttpResponseWithDefaultJsonHeader(): void
    {
        $response = new PaginationResponse(
            data: [new JsonResponseFixtureResource(name: 'Анна')],
            meta: new PaginationMetaResponse(nextCursor: 'next-cursor', limit: 20),
        );
        ...
        self::assertJsonStringEqualsJsonString(
            '{"data":[{"name":"Анна"}],"meta":{"nextCursor":"next-cursor","limit":20}}',
    ...
    public function testPaginationResponseCanOverrideStatusAndHeaders(): void
    {
        $response = new PaginationResponse(
            data: [],
            meta: new PaginationMetaResponse(nextCursor: null, limit: 20),
        )
            ->withHeader(new HttpHeaderValue(name: HttpHeader::XRequestId, value: 'request-1'))
            ->withAddedHeader(new HttpHeaderValue(name: HttpHeader::Vary, value: 'Accept'))
            ->withAddedHeader(new HttpHeaderValue(name: HttpHeader::Vary, value: 'Origin'))
    ...
    public function testHttpResponseInterceptorConvertsOpenApiResponse(): void
    {
        ...
        $httpResponse = $interceptor->intercept(
            context: self::createStub(CallContextInterface::class),
            handler: new JsonResponseFixtureHandler($response),
        );
    ...
    public function testHttpResponseInterceptorIgnoresOtherResults(): void
    {
        ...
        self::assertSame(
            ...
            $interceptor->intercept(
                context: self::createStub(CallContextInterface::class),
                handler: new JsonResponseFixtureHandler('plain result'),
            ),
        );
    }
}
...
{
    public function __construct(
        private ConvertsToHttpResponse|string $response,
    ) {}
