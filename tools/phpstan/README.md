...
Класс: `Tools\PHPStan\Rules\DisallowMagicScalarLiteralRule`

Что делает:

- Запрещает runtime magic scalar literals в `app/src`.
- Ловит non-empty strings, int кроме `0` и `1`, float, `true`, `false`.
- Требует вынести значение в class constant, enum, value object или другой именованный тип.
...
```php
return new HealthResource(status: HealthStatus::Ok);
```
...
- Class constants.
- Enum cases backing values.
- PHP attributes.
- Классы исключений с именем `*Exception`, потому что они часто собирают человекочитаемые сообщения и технические шаблоны ошибок.
- Значения по умолчанию параметров методов/конструкторов: `__construct(string $dsn = 'redis://redis:6379/0')`.
- Сообщения исключений: `new SomeException('Понятное сообщение')`.
- Сообщения response-классов: `new ErrorResponse(message: 'Понятное сообщение', status: HttpStatus::NotFound)`.
...
- По умолчанию правило анализирует только пути, содержащие `/app/src/`.
- В тестах можно передать другой список путей через constructor argument `analysedPathFragments`.
- Пустая строка разрешена.
- `0` и `1` разрешены как базовые технические значения.
- Если строка является ограниченным набором вариантов (`'ok'`, `'active'`, `'pending'`), нужен enum, а не просто константа.
- Сообщения логов, исключений и response-классов разрешены inline, потому что это человекочитаемый текст, а не управляющее значение.
